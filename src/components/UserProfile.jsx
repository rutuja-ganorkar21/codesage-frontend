...
function getMotivation(solved, total, accuracy) {
  ...
  if (accuracy >= 90)
    return "Outstanding accuracy! You're solving smart, not just hard.";
...
const getLang = (l) =>
  ({ javascript: "JS", java: "Java", cpp: "C++", "c++": "C++" })[
    l?.toLowerCase()
  ] || l;
const diffColor = (d) =>
  ({ easy: "#34d399", medium: "#fbbf24", hard: "#f87171" })[d?.toLowerCase()] ||
  "#94a3b8";
  ...
  return (
    <div
      style={{
        ...
        alignItems: "center",
        padding: "11px 0",
        borderBottom: last ? "none" : "1px solid #0d1828",
      }}
    >
  ...
  const stats = useMemo(() => {
    ...
    const byDiff = (diff) => ({
      solved: solvedProblems.filter((sp) =>
        problems.find(
          (p) => p._id === sp._id && p.difficulty?.toLowerCase() === diff,
        ),
      ).length,
      total: problems.filter((p) => p.difficulty?.toLowerCase() === diff)
        .length,
    });
    ...
    const accuracy = totalSubs
      ? Math.round((acceptedSubs / totalSubs) * 100)
      : 0;
    ...
    >
      ...
      >
        ...
        >
            ...
            >
              ...
              <span
                style={{
                  ...
                  border: `1px solid ${stats.rank.color}33`,
                  padding: "3px 10px",
                  borderRadius: "999px",
                  fontSize: "10px",
                  fontWeight: 700,
          ...
          {/* Divider */}
          <div
            style={{ height: "1px", background: "#0d1828", margin: "0 20px" }}
          />
          ...
          <div style={{ padding: "16px 20px" }}>
            ...
            >
              <span style={{ fontSize: "18px", flexShrink: 0 }}>
                {stats.rank.label.split(" ")[0]}
              </span>
              <span
                style={{ fontSize: "11px", color: "#475569", lineHeight: 1.7 }}
              >
                {stats.motivation}
              </span>
            </div>
          ...
          >
            ...
            {!loadingSub && submissions.length > 0 && (
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns: "1fr 110px 50px 65px",
                  gap: "12px",
                  padding: "8px 20px",
                  background: "#060b12",
            ...
            {loadingSub ? (
              ...
              <div style={{ padding: "48px", textAlign: "center" }}>
                ...
                <div style={{ fontSize: "13px", color: "#2d3f5c" }}>
              ...
              submissions.slice(0, 10).map((sub, i) => {
                ...
                return (
                  <div
                    key={sub._id}
                    onClick={() => {
                      if (sub.problemId?._id)
                        navigate(`/problem/${sub.problemId._id}`);
                    }}
                    style={{
                      display: "grid",
                      gridTemplateColumns: "1fr 110px 50px 65px",
                      ...
                      padding: "13px 20px",
                      borderBottom:
                        i < Math.min(submissions.length, 10) - 1
                          ? "1px solid #0d1828"
                          : "none",
                      cursor: sub.problemId?._id ? "pointer" : "default",
                    ...
                    >
                      ...
                      <span
                        style={{
                          fontSize: "13px",
                          fontWeight: 600,
                          color: sub.problemId?.title ? "#e2e8f0" : "#475569",
                          overflow: "hidden",
                          textOverflow: "ellipsis",
                          whiteSpace: "nowrap",
                          fontStyle: sub.problemId?.title ? "normal" : "italic",
                        }}
                      >
      ...
      {showPhotoModal && (
        <ProfilePicture onClose={() => setShowPhotoModal(false)} />
      )}
